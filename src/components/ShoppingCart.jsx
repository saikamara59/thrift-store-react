...
const ShoppingCart = () => {
  ...
  return (
    <main className="min-h-screen bg-blue-100">
      <NavBar />
      <h1 className="text-3xl font-bold mb-6 text-center p-8">Shopping Cart</h1>
      <div className="max-w-7xl mx-auto p-4">
        ...
        {cartItems.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7 shadow-md">
              {cartItems.map((item, index) => (
                <div key={item.id || index} className="bg-white rounded-lg shadow-md p-4 hover:shadow-md transition-shadow duration-300  max-w-xs">
                  <img src={item.image_url} alt={item.name} className="w-full h-48 object-cover rounded-md mb-4" />
                  <h3 className="text-xl font-semibold font-irish">{item.name}</h3>
            ...
            <div className="mt-8 bg-white rounded-lg shadow-md p-6 max-w-md mx-auto">
              <h2 className="text-2xl font-semibold mb-4 font-irish">Order Summary</h2>
              ...
              <p className="text-xl font-bold mt-4 font-irish"><strong>Total:</strong> ${total.toFixed(2)}</p>
