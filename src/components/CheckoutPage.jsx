...
const CheckoutPage = () => {
  const { cartItems, checkout, removeFromCart, updateQuantity } =
  ...
  return (
    <main className="bg-blue-100 min-h-screen">
      <NavBar />
      <h1 className="font-bold text-3xl text-center mb-6 p-5 font-irish">
        Checkout
      </h1>
      ...
      <section className="mb-8">
        ...
        {cartItems.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7 shadow-lg justify-between">
              {cartItems.map((item) => (
                <div
                  key={item.id}
      ...
      <section className="mb-8">
        <h2 className="text-xl font-semibold mb-4 font-irish">
          Payment Information
        </h2>
        <form className="grid grid-cols-1 gap-4 shadow-sm">
          {["cardholderName", "cardNumber", "expirationDate", "cvv"].map(
            (field) => (
              <div key={field}>
                <label className="block text-gray-700">
                  {field.charAt(0).toUpperCase() + field.slice(1)}
      ...
      <button
        ...
        className=" h-full bg-green-500 text-white py-2 px-6 rounded-md hover:bg-green-600"
      >
        Confirm Payment
      </button>
    </main>
